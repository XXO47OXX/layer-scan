...
def _compute_log_odds(
    ...
    log_odds = log P(correct) - log(1 - P(correct))
             = log_logits[correct] - logsumexp(log_logits[others])

    Returns:
        Tuple of (log_odds, is_correct). Returns (None, None) if
        correct_answer is not found in score_values.
    """
    # Find the index of the correct answer in score_values
    correct_idx = None
    for idx, val in enumerate(score_values):
        if int(val) == correct_answer:
            correct_idx = idx
    ...
    # Log-odds: log P(correct) - log(sum P(incorrect))
    log_softmax = F.log_softmax(restricted_logits, dim=0)
    log_p_correct = log_softmax[correct_idx].item()

    # log(1 - P(correct)) = log(sum P(other))
    # Use logsumexp over all tokens except correct
    ...
    # Accuracy: does argmax match correct?
...
def aggregate_scores_full(results: list[ScoreResult]) -> AggregateResult:
    ...
    # Compute log-odds and accuracy from samples that have them
